...
class InitialCostEstimator:
    ...
    def extract_initial_costs_from_ocr(self, extracted_text: str) -> Dict:
        ...
        if self.law_checker and self.law_checker.gemini_available:
            prompt = f"""以下の不動産広告のテキストから、初期費用に関する項目を抽出し、必ず金額（円）の整数で返してください。

【重要】月額賃料: {rent_value}円（この金額を使用して月数や割合を金額に変換してください）

## 抽出ルール（厳密に遵守）

### 1. 敷金（敷金、敷、敷引）
- 金額表記: 「敷金: 100,000円」→ 100000
- 月数表記: 「敷金2ヶ月」「敷2ヶ月」「敷金2か月」→ {rent_value} × 2
- 「なし」「無し」「0円」→ 0
- テキスト内を注意深く検索し、すべての表記パターンを確認してください

### 2. 礼金（礼金、礼）
- 金額表記: 「礼金: 50,000円」→ 50000
- 月数表記: 「礼金1ヶ月」「礼1ヶ月」→ {rent_value} × 1
- 「なし」「無し」「0円」→ 0
- 「礼金なし」と明記されている場合は必ず0としてください

### 3. 仲介手数料（仲介手数料、仲介、手数料）
- 金額表記: 「仲介手数料: 55,000円」→ 55000
- 月数表記: 「仲介手数料1.1ヶ月」→ {rent_value} × 1.1（小数点も可）
- 割合表記: 「賃料の1.1倍」→ {rent_value} × 1.1
- 「なし」「無し」「0円」→ 0

### 4. 保証会社（保証会社、保証金、初回保証料、保証料）
- 金額表記: 「保証会社: 25,000円」→ 25000
- 月数表記: 「保証金1ヶ月」→ {rent_value} × 1
- 割合表記: 「初回保証料:月額総賃料50%」「保証料:賃料の50%」→ {rent_value} × 0.5
- 「なし」「無し」「0円」→ 0
- 「毎月保証料」は初期費用ではないため無視してください（初回保証料のみ抽出）

### 5. 火災保険
- 金額表記: 「火災保険: 20,500円」→ 20500
- 「保険等 要2年20,500円」→ 20500（2年分でも初期費用として1年分または全額を返す）
- 「なし」「無し」「0円」→ 0

## 計算例（賃料: {rent_value}円の場合）
- 敷金2ヶ月 → {rent_value * 2 if rent_value > 0 else 0}
- 礼金なし → 0
- 仲介手数料1.1ヶ月 → {int(rent_value * 1.1) if rent_value > 0 else 0}
- 初回保証料:月額総賃料50% → {int(rent_value * 0.5) if rent_value > 0 else 0}
- 保険等 要2年20,500円 → 20500

## 抽出テキスト
{extracted_text}

## 出力形式
以下のJSON形式で、数値のみを返してください（説明文は不要）:
{{
  "敷金": 0,
  "礼金": 0,
  "仲介手数料": 0,
...
【重要】テキストを注意深く読み、すべての表記パターンを確認してから回答してください。JSONのみを返してください。"""
